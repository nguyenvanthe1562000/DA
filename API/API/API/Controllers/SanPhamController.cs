...
namespace QLBanDoGiaDung_API.Controllers
{
    ...
    public class SanPhamController : ControllerBase
    {
        ...
        public SanPhamController(ISanPhamBussiness newBusiness, IConfiguration configuration, IHostingEnvironment hostingEnvironment)
        {
        ...
        [HttpGet]
        public IEnumerable<SanPhamModel> GetRelated(string id)
        {

            var sp = _spBusiness.GetDataAll().FirstOrDefault(x => x.MaSanPham == id);
            return _spBusiness.GetDataAll().Where(x => x.MaLoai == sp.MaLoai).Take(6);
        }
        [Route("get-home")]
        [HttpGet]
        public IEnumerable<SanPhamModel> Gethome()
        {
            return _spBusiness.GetDataAll().Take(8).ToList();
        }
        [Route("get-home-two")]
        [HttpGet]
        public IEnumerable<SanPhamModel> GethomeTow()
        {
            return _spBusiness.GetDataAll().Take(6).ToList();
        }
        ...
        public IActionResult CreateProduct([FromBody] SanPhamModel model)
        {
            ...
            var check = _spBusiness.Create(model);
            return Ok(new { msg = check, data = model.MaSanPham });
        }
        [Route("up-file")]
        [HttpPost]
